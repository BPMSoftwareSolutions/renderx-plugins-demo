...
const handlerMetadata = {
  ...
  initializeBuild: {
    ...
    userStory: "As a build orchestrator, I want build initialization to complete in <100ms so that the pipeline starts quickly and developers get fast feedback.",
  ...
  compileSources: {
    ...
    userStory: "As a build system, I want source compilation to complete in <10 minutes so that developers get build feedback within a reasonable timeframe (target: < 5 min for 80% of builds).",
    businessValue: "Reduces developer feedback latency; enables tight feedback loops; supports CI/CD SLOs; improves developer productivity"
  },
  ...
  validateCompliance: {
    ...
    businessValue: "Enforces architecture consistency; prevents technical debt; enables governance automation; reduces review time"
  },
  ...
  publishArtifacts: {
    ...
    userStory: "As a release system, I want artifact publishing to complete in <2 minutes so that builds are available for deployment immediately.",
    businessValue: "Enables rapid deployment cycles; ensures artifact traceability; supports artifact retention policies; enables rollback"
  },
  ...
  detectAnomalies: {
    ...
    businessValue: "Reduces MTTR by 70%; prevents customer impact; enables proactive response; improves SLA compliance"
  },
  ...
  establishBaseline: {
    ...
    userStory: "As an analytics system, I want baseline establishment to complete in <5 minutes so that anomaly detection can be activated quickly after system changes.",
    businessValue: "Enables accurate anomaly detection; reduces false positives; provides statistical rigor; supports incident correlation"
  }
};

// Package paths - ACTUAL SSOT
const packages = [
  // JavaScript packages - Flat structure
  ...
  // JavaScript packages - Nested structure
...
function getHandlerMetadata(handler) {
  ...
  if (handlerMetadata[handler]) return handlerMetadata[handler];
  
  // Try case-insensitive match
  const key = Object.keys(handlerMetadata).find(k => k.toLowerCase() === handler.toLowerCase());
  if (key) return handlerMetadata[key];
  
  // Return default if not found
  return {
    scenario: handler.replace(/([A-Z])/g, ' $1').trim(),
    acceptanceCriteria: [
      `Given the ${handler} handler is triggered\nWhen the operation executes\nThen it completes successfully\nAnd the output is valid\nAnd any required events are published`,
      `Given valid input parameters\nWhen ${handler} processes them\nThen results conform to expected schema\nAnd no errors are thrown\nAnd telemetry is recorded`,
      `Given error conditions\nWhen ${handler} encounters an error\nThen the error is logged\nAnd appropriate recovery is attempted\nAnd the system remains stable`,
      `Given performance requirements\nWhen ${handler} executes\nThen latency is acceptable\nAnd throughput meets SLA\nAnd resource usage is within limits`,
      `Given compliance requirements\nWhen ${handler} operates\nThen all governance rules are followed\nAnd audit trails are maintained\nAnd no violations occur`
    ],
    userStory: `As a system, I want ${handler} to execute reliably and measurably so that dependent services can rely on consistent behavior and predictable performance.`,
    businessValue: `Enables reliable system operations; ensures consistent service delivery; supports SLA compliance and performance tracking`
  };
}

function enhanceBeat(beat, pkg) {
  const handlerName = typeof beat.handler === 'string' 
    ? beat.handler.split('#')[1] || beat.handler.split('.').pop()
    : beat.handler?.name || beat.title;
  
  const metadata = getHandlerMetadata(handlerName);
  ...
  return {
    ...
    testCase: beat.testCase || `should execute ${handlerName}`
  };
}
...
function enhanceMovement(movement, pkg) {
  return {
    ...movement,
    userStory: movement.userStory || `As a system user, I want ${movement.name} to execute reliably and measurably so that the system performs consistently.`,
    persona: movement.persona || "System Operator",
    businessValue: movement.businessValue || "Enables reliable system operations",
    beats: (movement.beats || []).map(beat => enhanceBeat(beat, pkg))
  };
}
...
function enhanceSymphony(symphonyPath, pkg) {
  try {
    ...
    symphony.userStory = symphony.userStory || `As a product stakeholder, I want the ${symphony.name} process to execute with measurable and auditable results so that business outcomes are predictable and verifiable.`;
    symphony.persona = symphony.persona || "Product Owner";
    symphony.businessValue = symphony.businessValue || "Enables measurable business value and operational excellence";
...
console.log(`  Enhanced with Gherkin AC & Measurable Stories: ${results.enhanced}`);
