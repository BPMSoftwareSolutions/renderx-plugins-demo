...
export class StageDomGuard {
  ...
  static install(): void {
    ...
    try {
      ...
      Node.prototype.replaceChild = function <T extends Node>(newChild: Node, oldChild: T): T {
        ...
        return StageDomGuard.originals.replaceChild!.call(this, newChild, oldChild);
      } as any;

      this.originals.insertBefore = Node.prototype.insertBefore;
      Node.prototype.insertBefore = function <T extends Node>(newNode: Node, ref: T | null): T {
      ...
      CSSStyleDeclaration.prototype.setProperty = function (prop: string, value: string | null, priority?: string) {
        StageDomGuard.maybeWarn("style.setProperty", (this as any).ownerElement);
        return StageDomGuard.originals.styleSetProperty!.call(this, prop, value, priority);
      } as any;

      // document.createElement (for visibility)
      this.originals.createElement = Document.prototype.createElement;
      Document.prototype.createElement = function (tagName: string, options?: any) {
        StageDomGuard.maybeWarn("document.createElement", this);
        return StageDomGuard.originals.createElement!.call(this, tagName, options);
      } as any;

      this.installed = true;
    } catch (e) {
      console.warn("⚠️ StageDomGuard: install failed:", (e as any)?.message || e);
    }
  }
  ...
  static uninstall(): void {
    ...
    try {
      ...
      if (this.originals.removeAttribute) Element.prototype.removeAttribute = this.originals.removeAttribute;
      if (this.originals.appendChild) Node.prototype.appendChild = this.originals.appendChild;
      if (this.originals.removeChild) Node.prototype.removeChild = this.originals.removeChild;
      if (this.originals.replaceChild) Node.prototype.replaceChild = this.originals.replaceChild;
      if (this.originals.insertBefore) Node.prototype.insertBefore = this.originals.insertBefore;
      if (this.originals.classListAdd) (DOMTokenList.prototype as any).add = this.originals.classListAdd;
      if (this.originals.classListRemove) (DOMTokenList.prototype as any).remove = this.originals.classListRemove;
      if (this.originals.styleSetProperty) CSSStyleDeclaration.prototype.setProperty = this.originals.styleSetProperty;
      if (this.originals.createElement) Document.prototype.createElement = this.originals.createElement;
    } catch {}
