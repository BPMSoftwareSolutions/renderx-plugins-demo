...
export function computeCssVarBlock(vars: Record<string, string | number> | undefined): string {
...
export function computeInlineStyle(data: any, tpl: any): Record<string, string> {
  const style: Record<string, string> = {};
  if (data?.position) {
    style.position = "absolute";
    style.left =
      typeof data.position.x === "number" ? `${data.position.x}px` : String(data.position.x);
    style.top =
      typeof data.position.y === "number" ? `${data.position.y}px` : String(data.position.y);
  }
  if (tpl?.dimensions) {
