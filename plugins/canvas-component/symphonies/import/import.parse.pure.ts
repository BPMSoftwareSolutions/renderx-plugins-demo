export function parseUiFile(_data: any, ctx: any) {
  try {
    ...
    if (!ui || typeof ui !== "object") throw new Error("No uiFileContent provided");
    if (!Array.isArray(ui.components)) throw new Error("uiFileContent.components must be an array");
    ...
    const comps = ui.components.map((c: any) => ({
      ...
      classRefs: Array.isArray(c.template?.classRefs) ? c.template.classRefs.slice() : [],
      style: typeof c.template?.style === "object" && c.template?.style ? c.template.style : {},
      layout: {
