...
describe('Gallery sample integration (sanitization smoke)', () => {
  ...
  it('retains animate / gradient elements where present', async () => {
    const animated = await loadSample('../../docs/samples/svg-sample-line-chart-animated.html');
    const pulse = await loadSample('../../docs/samples/svg-sample-animated-pulse.html');
    const grad = await loadSample('../../docs/samples/svg-sample-bar-chart.html');
