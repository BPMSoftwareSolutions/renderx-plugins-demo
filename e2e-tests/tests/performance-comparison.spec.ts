...
async function measurePerformance(
  ...
  const metrics = await page.evaluate(() => {
    ...
    const cacheStats = window.MusicalConductorCache
    ...
    return {
      ...
      moduleLoadTime: perfMetrics.moduleLoadEnd
        ? perfMetrics.moduleLoadEnd - perfMetrics.moduleLoadStart
        : undefined,
...
test.describe("MusicalConductor Performance Comparison", () => {
  test("should demonstrate performance improvement with module caching", async ({
    page,
  }) => {
    ...
    console.log(
      `Original Load Time:        ${originalMetrics.totalLoadTime.toFixed(2)}ms`
    );
    console.log(
      `Cached Load Time:          ${cachedMetrics.totalLoadTime.toFixed(2)}ms`
    );

    if (cachedMetrics.cacheHitRatio !== undefined) {
      console.log(
        `Cache Hit Ratio:           ${cachedMetrics.cacheHitRatio.toFixed(1)}%`
      );
    ...
    const requestReduction =
      ((originalMetrics.httpRequests - cachedMetrics.httpRequests) /
        originalMetrics.httpRequests) *
      100;
    const timeImprovement =
      ((originalMetrics.totalLoadTime - cachedMetrics.totalLoadTime) /
        originalMetrics.totalLoadTime) *
      100;
    ...
    if (
      cachedMetrics.cacheHitRatio !== undefined &&
      cachedMetrics.cacheHitRatio > 0
    ) {
  ...
  test("should test cached version multiple times to verify caching benefits", async ({
    page,
  }) => {
    const runs = 3;
    ...
    console.log(
      `🔄 Running cached version ${runs} times to test caching effectiveness...`
    );
    ...
    if (
      lastRun.cacheHitRatio !== undefined &&
      firstRun.cacheHitRatio !== undefined
    ) {
      expect(lastRun.cacheHitRatio).toBeGreaterThanOrEqual(
        firstRun.cacheHitRatio
      );
  ...
  test("should verify conductor functionality in cached version", async ({
    page,
  }) => {
...
test.describe("Performance Issue Demonstration", () => {
  test("should demonstrate the current performance problems", async ({
    page,
  }) => {
    ...
    page.on("request", (request) => {
      if (
        request.url().includes("/dist/modules/") ||
    ...
    await page.goto("http://127.0.0.1:3000/bundled");
    await page.waitForLoadState("networkidle");
    ...
    if (moduleRequests.length > 10) {
      console.log(
        `  ... and ${moduleRequests.length - 10} more module requests`
      );
    }
    ...
    console.log(
      `Total requests per suite:  ${totalRequestsPerSuite.toLocaleString()}`
    );
    console.log("─".repeat(60));
