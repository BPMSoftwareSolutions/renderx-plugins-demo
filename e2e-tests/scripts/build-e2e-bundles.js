...
function printBuildSummary(stats) {
  ...
  console.log(
    `  Total Bundle Size:              ${formatSize(totalSize).padStart(10)}`
  );
  ...
  const improvement = (
    ((beforeRequests - afterRequests) / beforeRequests) *
    100
  ).toFixed(1);
...
function generateImportMap(stats) {
  ...
  // Map main bundle (ES module)
  const mainBundle = assets.find(
    (name) => name === "musical-conductor.bundle.js"
  );
  if (mainBundle) {
