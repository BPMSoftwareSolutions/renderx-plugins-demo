...
Desktop missing 10 text labels/content that web displays
...
Desktop missing 4 UI elements that web renders: code (expected code), button (expected Button), h4 (expected h4), pre (expected pre)

- **Web:** Web renders: button, code, div, h4, p, pre, span
- **Desktop:** Desktop renders: Border, StackPanel, TextBlock, UserControl
...
- **Web:** Web shows: Component JSON, ✏️ Edit, View/Hide JSON, Generate a new version, ➕ Add to Library...
- **Desktop:** Desktop shows: Message content, User, 💬
...
- **Desktop:** Desktop renders: Border, Ellipse, StackPanel, TextBlock, UserControl
- **Impact:** Users see incomplete or different UI structure than web version
...
- **Props:** setShowAIChat] = React.useState(false);
  const safeItems = Array.isArray(items) ? items, {
          onComponentsLoaded
- **Hooks:** useConductor, useCallback, useEffect, React.useEffect, useState
- **CSS Classes:** 18
