...
- **Web:** Web shows: Copy JSON to clipboard, Generate a new version, Add to Component Library, Edit component, View/Hide JSON...
- **Desktop:** Desktop shows: 🧩, 🔄 Try Again, Component JSON, 📋 Copy JSON, ✕
...
AI chat toggle is implemented in ChatMessage.axaml, ChatWindow.axaml, LibraryPanel.axaml but expected in LibraryPlugin.axaml.
...
AI availability hint is implemented in ChatWindow.axaml, LibraryPanel.axaml but expected in LibraryPlugin.axaml.
...
- **Props:** setShowAIChat] = React.useState(false);
  const safeItems = Array.isArray(items) ? items, {
          onComponentsLoaded
- **Hooks:** useEffect, useState, React.useState, React.useEffect, React.useCallback
- **CSS Classes:** 18
