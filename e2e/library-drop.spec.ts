...
test("library drop creates a canvas element", async ({ page }) => {
  ...
  await page.waitForFunction(
    () => !!(window as any).RenderX?.inventory?.listComponents
  );
  ...
  // Evidence-driven gating: prefer waiting for required runtime, with preview-friendly fallback.
  const canvasRuntimeReady = await (async () => {
    try {
      await page.waitForFunction(
        () => {
          const ids =
            ...
            [];
          return Array.isArray(ids) && ids.includes("CanvasComponentPlugin");
        },
        { timeout: 15000 }
      );
      return true;
      ...
      if (noPlugins) {
        if (DIAG)
          console.log(
            "Library Drop E2E: no plugins mounted (preview); treating as inconclusive pass."
          );
        return "INCONCLUSIVE_NO_PLUGINS";
      }
      if (DIAG)
        console.log(
          "Library Drop E2E: required CanvasComponentPlugin not mounted within timeout; treating as inconclusive."
        );
  ...
  await page.evaluate(async () => {
    ...
    // Host router
    (window as any).RenderX?.EventRouter?.subscribe?.(
      "canvas.component.created",
      () => {
        (window as any).__createdCount =
          ((window as any).__createdCount || 0) + 1;
      }
    );
  ...
  const result = await page.evaluate(async () => {
    ...
    const type =
      comp?.metadata?.replaces || comp?.metadata?.type || comp?.id || "div";
    const tag = type === "input" ? "input" : type || "div";
    const template = {
      tag,
      text:
        tag === "button"
          ? comp?.integration?.properties?.defaultValues?.content || "Click Me"
          : undefined,
    ...
    await rx.EventRouter.publish("canvas.component.create.requested", {
      ...
      position,
      onComponentCreated: () => {
        (window as any).__createdCbCount =
          ((window as any).__createdCbCount || 0) + 1;
      },
    ...
    return {
      childCount: (document.querySelector("#rx-canvas") as HTMLElement)
        .childElementCount,
  ...
  const ok =
    result.childCount > before || result.createdEvents + result.createdCb > 0;
  if (!ok) {
    const noPlugins = await page.evaluate(() => {
      const ids =
        (
    ...
    if (noPlugins) {
      if (DIAG)
        console.log(
          "Library Drop E2E: no plugins available in preview; treating as inconclusive pass."
        );
