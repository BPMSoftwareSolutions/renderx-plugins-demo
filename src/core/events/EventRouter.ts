...
export const EventRouter = {
  ...
  async publish(topic: string, payload: any, conductor?: any) {
    if (typeof window !== "undefined") {
      (window as any).__DEBUG_EVENTROUTER =
        (window as any).__DEBUG_EVENTROUTER || [];
    ...
    if (!def && typeof window !== "undefined") {
      ...
      if (globalGetTopicDef) {
        ...
        if (typeof window !== "undefined")
          (window as any).__DEBUG_EVENTROUTER.push(
            `used_global_getTopicDef(${topic}): ${!!def}`
          );
        try {
          console.log(
            `[EventRouter] Used global getTopicDef for '${topic}', found: ${!!def}`
          );
        } catch {}
      }
    }
    ...
    if (typeof window !== "undefined")
      (window as any).__DEBUG_EVENTROUTER.push(
        `found_topic_def(${topic}): routes=${def.routes?.length || 0}`
      );
    try {
      console.log(`[EventRouter] Topic '${topic}' definition:`, {
    ...
    if (__publishStack.includes(topic)) {
      try {
        console.warn(
          `[topics] Blocking immediate republish of '${topic}' to prevent feedback loop`
        );
      } catch {}
      return;
    }
    try {
      if (isFlagEnabled("lint.topics.runtime-validate") && def.payloadSchema) {
        ...
        if (!validate(payload)) {
          ...
          const msg = errs
            .map((e: any) =>
              `${e.instancePath || e.dataPath || ""} ${e.message || ""}`.trim()
            )
            .join("; ");
    ...
    const resolvedConductor =
      conductor && typeof conductor.play === "function"
        ? conductor
        ...
        : undefined;
    try {
      if (topic && topic.startsWith("library.component.")) {
        const globalC =
          typeof window !== "undefined"
            ? (window as any).RenderX?.conductor
            : undefined;
        const isSame = !!(
          resolvedConductor &&
          globalC &&
          resolvedConductor === globalC
        );
        const ids =
          resolvedConductor &&
          typeof (resolvedConductor as any).getMountedPluginIds === "function"
            ? (resolvedConductor as any).getMountedPluginIds()
            : [];
        console.log(
          `[EventRouter] Conductor probe for '${topic}': sameAsGlobal=${isSame}, mountedCount=${
            Array.isArray(ids) ? ids.length : "n/a"
          }`,
          ids
        );
      }
    ...
    let deliver = async (p: any) => {
      if (typeof window !== "undefined")
        (window as any).__DEBUG_EVENTROUTER.push(
          `deliver_start(${topic}): routes=${def.routes?.length || 0}`
        );
      try {
        console.log(
          `[EventRouter] Starting delivery for '${topic}' with ${
            def.routes?.length || 0
          } routes`
        );
      } catch {}
      for (const r of def.routes as TopicRoute[]) {
        if (typeof window !== "undefined")
          (window as any).__DEBUG_EVENTROUTER.push(
            `routing_to: ${r.pluginId}::${r.sequenceId}`
          );
        try {
          const hasPlay = !!(
            resolvedConductor && typeof resolvedConductor.play === "function"
          );
          try {
            console.log(
              `[topics] Routing '${topic}' -> ${r.pluginId}::${r.sequenceId} (hasPlay=${hasPlay})`
            );
          } catch {}
          await resolvedConductor?.play?.(r.pluginId, r.sequenceId, p);
          if (typeof window !== "undefined")
            (window as any).__DEBUG_EVENTROUTER.push(
              `routed_success: ${r.pluginId}::${r.sequenceId}`
            );
        } catch (e) {
          if (typeof window !== "undefined")
            (window as any).__DEBUG_EVENTROUTER.push(
              `route_error: ${r.pluginId}::${r.sequenceId} - ${e}`
            );
          try {
            console.warn(
              `[topics] Failed to route '${topic}' -> ${r.pluginId}::${r.sequenceId}:`,
              e
            );
          } catch {}
        }
