...
export function mapJsonComponentToTemplate(json) {
    ...
    if (icon?.mode === "emoji" && icon?.value) {
        ...
        if (icon.position)
            attrs["data-icon-pos"] = String(icon.position);
    }
    ...
    if (overlayKind)
        attrs["data-overlay"] = String(overlayKind);
    ...
    if (resize?.enabled === false)
        attrs["data-resize-enabled"] = "false";
    const handles = Array.isArray(resize?.handles)
        ? resize.handles.filter((h) => typeof h === "string")
        : [];
    if (handles.length)
        attrs["data-resize-handles"] = handles.join(",");
    const minW = resize?.constraints?.min?.w ??
        json?.integration?.canvasIntegration?.minWidth;
    const minH = resize?.constraints?.min?.h ??
        json?.integration?.canvasIntegration?.minHeight;
    if (minW != null)
        attrs["data-resize-min-w"] = String(minW);
    if (minH != null)
        attrs["data-resize-min-h"] = String(minH);
    // Default text content remains type-specific for now; will move to content rules later
    const defaults = json?.integration?.properties?.defaultValues || {};
    return {
        tag,
        text: type === "button"
...
//# sourceMappingURL=jsonComponent.mapper.js.map