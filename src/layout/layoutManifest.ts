...
export async function loadLayoutManifest(): Promise<any> {
  ...
  try {
    ...
    // Node/tests fallback: raw import from public
    const mod = await import(/* @vite-ignore */ "../../public/layout-manifest.json?raw");
...
export function validateLayoutManifest(m: any) {
  ...
  if (!m.layout || !m.slots) throw new Error("Missing layout or slots in manifest");
  return true;
}
