  ...
  try {
    ...
    try {
      const envMod = await import(
        /* @vite-ignore */ "../../core/environment/env"
      );
    ...
    try {
      const envMod2 = await import(
        /* @vite-ignore */ "../../core/environment/env"
      );
      const dir2 = envMod2.getArtifactsDir?.();
      if (dir2) return { plugins: [] };
      if ((envMod2 as any).allowFallbacks?.()) {
        // Prefer catalog-generated manifest in repo (CI/tests), then try public/ path.
        try {
          const catPath = [
            "..",
            "..",
            ...
            "json-plugins",
            ".generated",
            "plugin-manifest.json?raw",
          ].join("/");
          const mod1: any = await import(/* @vite-ignore */ catPath as any);
          const text1: string = mod1?.default || mod1 || '{"plugins":[]}';
        ...
        try {
          ...
          const mod2: any = await import(/* @vite-ignore */ pubPath as any);
          const text2: string = mod2?.default || mod2 || '{"plugins":[]}';
