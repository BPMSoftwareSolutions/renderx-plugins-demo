...
const ALLOWED_TAGS = new Set([
  'a','b','strong','i','em','u','s','p','br','span','ul','ol','li','h1','h2','h3','h4','h5','h6',
  'blockquote','code','pre','img','svg','path','rect','circle','g','div',
  'form','button','input','label','table','thead','tbody','tr','th','td',
  // Extended SVG support for richer samples
  'text','line','polyline','polygon','ellipse','defs','lineargradient','stop','animate','animatetransform'
]);
...
export function sanitizeHtml(input: string): string {
  ...
  while (walker.nextNode()) {
    ...
    for (const attr of Array.from(el.attributes)) {
      ...
      if (name === 'style') { // Optional: keep? For now allow basic inline styles but strip expressions
