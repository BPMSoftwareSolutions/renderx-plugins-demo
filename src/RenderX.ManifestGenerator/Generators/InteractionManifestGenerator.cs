...
/// <summary>
/// Generates interaction manifest from sequence files.
/// Equivalent to web's generate-interaction-manifest.js
/// </summary>
public class InteractionManifestGenerator
{
    private readonly string _pluginsDir;
    private readonly JsonSerializerOptions _jsonOptions;

    public InteractionManifestGenerator(string pluginsDir, JsonSerializerOptions jsonOptions)
    {
        _pluginsDir = pluginsDir;
        _jsonOptions = jsonOptions;
    }

    public async Task<InteractionManifest> GenerateAsync()
    {
        var manifest = new InteractionManifest
        {
        ...
        foreach (var file in sequenceFiles)
        {
            try
            {
                ...
                // Extract events (routes) from sequence
                foreach (var movement in sequence.Movements)
                {
                    ...
                    foreach (var beat in movement.Beats)
                    {
                        ...
                        if (!manifest.Routes.ContainsKey(beat.Event))
                        {
                            manifest.Routes[beat.Event] = new RouteInfo
                            {
