...
const mappingPath = path.join(assetsDir, 'scene6_boundaries.mapping.json');

describe('Scene 6: Boundaries (Guard Rails)', () => {
  it('matches mapping and contains guard rails + boundary labels + animated bus', () => {
    const mapping = JSON.parse(fs.readFileSync(mappingPath, 'utf-8'));
    expect(mapping.scene.name).toBe('scene6_boundaries');
    expect(mapping.output.file).toBe('integrated_scene_6.svg');
    ...
    // Guard rails present
    expect(doc.getElementById(mapping.boundaries.id)).toBeTruthy();
    ...
    expect(textContent).toMatch(/BOUNDARIES ENFORCED/);
    expect(textContent).toMatch(/GUARD RAILS/);
  });
});
