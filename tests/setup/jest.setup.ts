...
expect.extend({
  toBeWithinMusicalTiming(received: number, expected: number, tolerance: number = 50) {
    const pass = Math.abs(received - expected) <= tolerance;
    
    if (pass) {
      return {
        message: () => `Expected ${received}ms not to be within ${tolerance}ms of ${expected}ms`,
        pass: true,
      };
    } else {
      return {
        message: () => `Expected ${received}ms to be within ${tolerance}ms of ${expected}ms (difference: ${Math.abs(received - expected)}ms)`,
        pass: false,
      };
    }
  },
  ...
  toHaveEventSubscription(eventBus: any, eventName: string) {
    ...
    if (hasSubscription) {
      return {
        message: () => `Expected EventBus not to have subscription for '${eventName}'`,
        pass: true,
      };
    } else {
      return {
        message: () => `Expected EventBus to have subscription for '${eventName}', but found subscriptions for: ${Object.keys(debugInfo.subscriptionCounts).join(', ')}`,
        pass: false,
      };
    }
  },
  ...
  toHaveSequenceRegistered(conductor: any, sequenceName: string) {
    ...
    if (hasSequence) {
      return {
        message: () => `Expected MusicalConductor not to have sequence '${sequenceName}' registered`,
        pass: true,
      };
    } else {
      return {
        message: () => `Expected MusicalConductor to have sequence '${sequenceName}' registered, but found: ${sequences.join(', ')}`,
        pass: false,
      };
    }
  },
  ...
  toHaveSPAViolation(validator: any, violationType: string) {
    ...
    if (hasViolationType) {
      return {
        message: () => `Expected SPAValidator not to have '${violationType}' violations`,
        pass: true,
      };
    } else {
      return {
        message: () => `Expected SPAValidator to have '${violationType}' violations, but found: ${JSON.stringify(report.violationsBySeverity)}`,
        pass: false,
      };
    }
  }
