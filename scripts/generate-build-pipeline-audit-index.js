#!/usr/bin/env node
/**
 * Generate Build Pipeline Audit Index
 * 
 * Purpose: Create BUILD_PIPELINE_AUDIT_INDEX.md from buildPipelineAudit.index
 * Source: orchestration-audit-system-project-plan.json
 * Output: BUILD_PIPELINE_AUDIT_INDEX.md
 * 
...
const OUTPUT_PATH = path.join(rootDir, 'BUILD_PIPELINE_AUDIT_INDEX.md');

function generateAuditIndex() {
  const plan = JSON.parse(fs.readFileSync(PLAN_PATH, 'utf-8'));
  const index = plan.governanceDocumentation.buildPipelineAudit.index;
