...
function load(name) {
  try {
    return JSON.parse(readFileSync(join(artifactsDir, name), "utf-8"));
  } catch {
    return null;
  }
}
...
try {
  pluginManifest = JSON.parse(
    readFileSync(
      join(root, "public", "plugins", "plugin-manifest.json"),
      "utf-8"
    )
  );
} catch {}
try {
  if (!pluginManifest)
    pluginManifest = JSON.parse(
      readFileSync(
        join(root, "catalog", "json-plugins", "plugin-manifest.json"),
        "utf-8"
      )
    );
} catch {}
...
const allowlist = (process.env.RENDERX_SEQUENCE_COVERAGE_ALLOW || "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
const missingSequences = Array.from(runtimePluginIds).filter(
  (id) =>
    !allowlist.includes(id) &&
    !Array.from(sequencePluginIds).some((seqId) =>
      seqId.toLowerCase().includes(
        id
          .replace(/plugin$/i, "")
          .toLowerCase()
          .replace(/[-]/g, "")
...
if (missingSequences.length) {
  console.warn(
    "⚠️ Plugins without obvious sequence coverage (heuristic):",
    missingSequences
  );
  globalThis.__artifactWarnings = (globalThis.__artifactWarnings || []).concat([
    { type: "coverage", items: missingSequences },
  ]);
}
if (!failed)
  console.log(
    "✅ Artifacts validation passed (schemaVersion",
    schemaVersion,
    ")"
  );
...
if (strict && (globalThis.__artifactWarnings?.length || 0) > 0) {
  console.error(
    `RENDERX_VALIDATION_STRICT=1 escalating ${globalThis.__artifactWarnings.length} warnings to error.`
  );
  process.exit(1);
}
...
if (process.env.RENDERX_REQUIRE_SIGNATURE === "1") {
  try {
    ...
    if (!ok) {
      console.error(
        "❌ Signature verification failed under RENDERX_REQUIRE_SIGNATURE=1"
      );
      process.exit(1);
    }
    ...
    console.error(
      "❌ Required signature missing or unreadable:",
      e?.message || e
    );
    process.exit(1);
  }
