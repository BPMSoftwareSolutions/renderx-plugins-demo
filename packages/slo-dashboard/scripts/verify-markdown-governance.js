...
function loadAuthority() {
  try {
    ...
    console.error('❌ Failed to load governance authority:', error.message);
    process.exit(1);
  }
}
...
function isAutoGeneratedFile(filePath) {
  try {
    ...
    return content.includes('AUTO-GENERATED FILE') && 
...
function verifyMarkdownGovernance(_options = {}) {
  console.log('\n🔍 Markdown Governance Verification');
  ...
  console.log('📋 Scanning .generated/ directory...\n');
  ...
  files.forEach(file => {
    ...
    if (CONFIG.mustBeAutoGenerated.includes(file)) {
      if (!isAutoGen) {
        ...
        console.log(`❌ ${file} - MANUAL CREATION (violation)`);
      } else {
        compliant.push(file);
        console.log(`✅ ${file} - Auto-generated (compliant)`);
      }
    }
  });
  ...
  files.forEach(file => {
    ...
    if (!isAutoGen) {
      ...
      console.log(`⚠️  ${file} - UNAUTHORIZED MANUAL FILE (violation)`);
    }
  });
  ...
  if (violations.length > 0) {
    ...
    console.log('✅ ALL MARKDOWN FILES COMPLY WITH GOVERNANCE POLICY\n');
  }
...
function autoFixViolations(verificationResult) {
  ...
  try {
    ...
    console.log('\n✅ All markdown files regenerated\n');
  } catch (error) {
