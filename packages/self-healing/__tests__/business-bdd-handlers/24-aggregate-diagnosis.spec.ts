...
describe.skip('Business BDD: aggregateDiagnosis', () => {
  let _ctx: any;
  ...
  describe('Scenario: Combine multiple diagnoses into coherent report', () => {
    it('should achieve the desired business outcome', async () => {
      ...
      // - multiple diagnoses completed
      // - results span multiple issues

      // TODO: Set up preconditions
      // ctx.input = { /* realistic production data */ };

      // WHEN (Action - User/System Action)
      // - aggregation handler processes diagnoses

      // TODO: Execute handler
      // ctx.output = await ctx.handler(ctx.input);

      // THEN (Expected Outcome - Business Value)
      // - diagnoses should be grouped
