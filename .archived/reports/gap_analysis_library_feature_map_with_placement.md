...
Desktop missing 3 UI elements that web renders: pre (expected pre), code (expected code), h4 (expected h4)
...
- **Web:** Web shows: View/Hide JSON, Copy JSON to clipboard, Generate a new version, Hide JSON, Edit component...
- **Desktop:** Desktop shows: User, 📄 View JSON, 🔄 Try Again, ✏️ Edit, 💬
...
- **Props:** setShowAIChat] = React.useState(false);
  const safeItems = Array.isArray(items) ? items, {
          onComponentsLoaded
- **Hooks:** React.useState, useConductor, useCallback, React.useEffect, useState
- **CSS Classes:** 18
